...
var _default = (snapshots, globalConfig, updateCommand) => {
  ...
  if (snapshots.added) {
    summary.push(
      SNAPSHOT_ADDED(
        ARROW + (0, _utils.pluralize)('snapshot', snapshots.added) + ' written '
      ) + `from ${(0, _utils.pluralize)('test suite', snapshots.filesAdded)}.`
    );
  }
  ...
  if (snapshots.unmatched) {
    summary.push(
      FAIL_COLOR(
        `${ARROW}${(0, _utils.pluralize)(
          'snapshot',
          snapshots.unmatched
        )} failed`
      ) +
        ` from ${(0, _utils.pluralize)(
          'test suite',
  ...
  if (snapshots.updated) {
    summary.push(
      SNAPSHOT_UPDATED(
        ARROW +
          (0, _utils.pluralize)('snapshot', snapshots.updated) +
          ' updated '
      ) + `from ${(0, _utils.pluralize)('test suite', snapshots.filesUpdated)}.`
    );
  }
    ...
    if (snapshots.didUpdate) {
      summary.push(
        SNAPSHOT_REMOVED(
          ...
          `from ${(0, _utils.pluralize)('test suite', snapshots.filesRemoved)}.`
      );
    } else {
      summary.push(
        OBSOLETE_COLOR(
