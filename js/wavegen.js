...
function waveGen(hex) {
	...
	this.circleGeneration = function() {
        var random_color = randInt(0, colors.length);
		if (this.dt - this.lastGen > this.nextGen + 500) {
			...
			for (var i = 0; i < MainHex.sides; i++) {
				addNewBlock(i, colorList[random_color], 1.5 + (this.difficulty / 15) * 3);
			}
	...
	this.crosswiseGeneration = function() {
		...
		if (this.dt - this.lastGen > this.nextGen) {
			...
			addNewBlock(i, colors[random_color], 0.6 + (this.difficulty / 15) * 3);
			addNewBlock((i + 3) % MainHex.sides, colors[random_color], 0.6 + (this.difficulty / 15) * 3);
			this.ct += 1.5;
			this.lastGen = this.dt;
			this.shouldChangePattern();
		}
	...
	this.spiralGeneration = function() {
		...
		if (this.dt - this.lastGen > this.nextGen * (2 / 3)) {
			if (dir) {
				addNewBlock(5 - (this.ct % MainHex.sides), colors[random_color], 1.5 + (this.difficulty / 15) * (3 / 2));
			} else {
				addNewBlock(this.ct % MainHex.sides, colors[random_color], 1.5 + (this.difficulty / 15) * (3 / 2));
			}
			this.ct += 1;
			this.lastGen = this.dt;
			this.shouldChangePattern();
	...
	this.doubleGeneration = function() {
		...
		if (this.dt - this.lastGen > this.nextGen) {
			var i = randInt(0, colors.length);
			addNewBlock(i, colors[random_color], 1.5 + (this.difficulty / 15) * 3);
			addNewBlock((i + 1) % MainHex.sides, colors[random_color], 1.5 + (this.difficulty / 15) * 3);
			this.ct += 2;
			this.lastGen = this.dt;
			this.shouldChangePattern();
		}
